...
function Products() {
  ...
  async function getCategoryData() {
    try {
      setIsLoading(true)
      const store = await fetch("/api/user/current-user", {
        method: "GET",
        headers: {
      ...
      const response = await fetch(`/api/product/get-data/${storeData.data.store._id}`, {
        method: "GET",
        headers: {
  ...
  const deleteAccount = async (e) => {
    try {
      const response = await fetch(`/api/product/delete/${deleteProductId}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        }
      ...
      <section className='bg-gray-100 flex-grow h-dvh min-w-80 lg:h-dvh lg:pb-0 pb-20'>
        <div className='lg:my-10 my-5 mx-4'>
          <div className='flex justify-between lg:justify-start lg:gap-5'>
            <h2 className='text-3xl text-zinc-900 font-extrabold ml-4 tracking-tight'>All Products</h2>
                    ...
                    <tbody>
                      {storeProducts.map((product, index) => {
                        return <tr key={index} className="border-b border-opacity-20 border-gray-300 bg-gray-50">
                          <td className="p-3">
                            <img className='h-10 w-10' src={"/uploads/" + product?.images.featuredImage || "./vite.svg"} alt="product" />
                          </td>
