...
function Category() {
  ...
  async function getCategoryData() {
    try {
      ...
      const response = await fetch(`/api/category/get-data/${storeData.data.store._id}`, {
        method: "GET",
        headers: {
                    ...
                    {categories.map((category, index) =>
                      <tr key={index} className="border-b border-opacity-20 border-gray-300 bg-gray-50">
                        <td className="p-3 text-base tracking-tight">
                          <img className='h-10 w-10' src={"/uploads/" + category.image} alt="" />
                        </td>
                        <td className="p-3 text-base tracking-tight">
                          <p>{category.name}</p>
                        </td>
                        ...
                        <td className="p-3 text-base tracking-tight">
                          <Link to={"../edit-category/"+category._id}>
                            <button className="px-4 py-1 font-semibold rounded-md bg-blue-600 text-gray-50">
                              Edit
                            </button>
                          </Link>
                        </td>
