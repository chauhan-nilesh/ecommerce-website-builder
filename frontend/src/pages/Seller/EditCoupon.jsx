...
const EditCoupon = () => {
    ...
    async function getCouponData() {
        try {
            setIsLoading(true)
            const response = await fetch(`/api/coupon/data/${id}`, {
                method: "GET",
                headers: {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.patch(`/api/coupon/edit/${id}`, formData, {
                headers: {
                    'Content-Type': 'application/json'
                }
            });
        ...
        <div className="bg-white lg:bg-gray-100 p-4 lg:p-10 lg:min-h-screen h-full lg:mb-0 mb-20">
            <div className="max-w-3xl mx-auto p-2 lg:p-6 bg-white lg:shadow-md rounded-md">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <button type="submit" className="w-full py-2 px-4 bg-blue-500 text-white font-bold rounded-md hover:bg-blue-600">Update Coupon</button>
                </form>
            </div>
        </div>
    );
