...
function Product() {
  ...
  async function getProductData() {
    try {
      setIsLoading(true);
      const store = await fetch(`/api/store/subdomain/${subdomain}`);
        ...
        <div className='grid grid-flow-row lg:grid-cols-2 h-full w-full'>
          <div className='w-full md:block lg:pb-9 pl-10 hidden'>
            <div className='w-full flex justify-center'>
              <img className='md:h-[500px] lg:h-[600px] w-full' src={"/uploads/" + defaultImage} alt={product.name} />
            </div>
            <div className='md:flex mt-5 justify-center'>
              {Object.values(product.images).map((image, idx) => (
                image ? <img key={idx} onClick={() => handleImage(image)} className={`h-24 w-auto ml-3 ${defaultImage === image ? 'border-2 border-black' : ''}`} src={"/uploads/" + image} alt={product.name} /> : null
              ))}
            </div>
          </div>
          ...
          <div className="md:hidden carousel carousel-center max-w-fit p-2 gap-2 bg-white rounded-box">
            {Object.values(product.images).map((image, idx) => (
              image ? <div key={idx} className="carousel-item"><img src={"/uploads/" + image} className="h-[420px] sm:h-[550px] rounded-box" alt={product.name} /></div> : null
            ))}
          </div>
