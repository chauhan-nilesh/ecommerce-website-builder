                ...
                if (product?.status === true && product?.stockStatus === true) {
                    return <div key={index} className="relative w-full max-w-xs overflow-hidden rounded-lg bg-white shadow-md">
                        <Link className='hover:opacity-75' to={"/product/"+product._id}>
                            <img className="h-fit w-auto rounded-t-lg object-cover" src={`${import.meta.env.VITE_API_URL}/uploads/` + product.images.featuredImage} alt="product image" />
                        </Link>
                        ...
                        <div className="mt-4 px-3 lg:px-5 pb-5">
                            ...
                            <div className="lg:flex items-center justify-between">
                                ...
                                <button type='button' onClick={() => addToCart({ ...product, quantity: 1 })} className="flex items-center justify-center rounded-md px-5 py-2.5 text-center text-sm font-medium hover:opacity-75 focus:outline-none"
